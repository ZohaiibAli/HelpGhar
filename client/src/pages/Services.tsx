...
export default function ServicesPage() {
  ...
  const [tab, setTab] = useState<TabId>(
    () => (searchParams.get("tab") === "jobs" ? "jobs" : "workers")
  );
  ...
  useEffect(() => {
    ...
    setTab(searchParams.get("tab") === "jobs" ? "jobs" : "workers");
  }, [searchParams]);
      ...
      <section className="border-b border-border bg-hero-gradient">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <h1 className="font-display text-3xl font-semibold tracking-tight md:text-4xl">
            {tab === "jobs" ? "Open job requests" : activeCat === "All" ? "All workers" : activeCat}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {tab === "jobs"
              ? "Jobs posted by customers, ready for a worker to apply."
              : "Filter by category, rating, price and availability. Prices are set by the workers themselves."}
          </p>
